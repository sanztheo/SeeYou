...
export const FLIR_SHADER = /* glsl */ `
  ...
  void main() {
    ...
    float px = 1.0 / 1024.0;
    vec4 color = texture2D(colorTexture, uv) * 0.25
      + texture2D(colorTexture, uv + vec2(-px, -px)) * 0.0625
      + texture2D(colorTexture, uv + vec2( 0.0, -px)) * 0.125
      + texture2D(colorTexture, uv + vec2( px, -px)) * 0.0625
      + texture2D(colorTexture, uv + vec2(-px,  0.0)) * 0.125
      + texture2D(colorTexture, uv + vec2( px,  0.0)) * 0.125
      + texture2D(colorTexture, uv + vec2(-px,  px)) * 0.0625
      + texture2D(colorTexture, uv + vec2( 0.0,  px)) * 0.125
      + texture2D(colorTexture, uv + vec2( px,  px)) * 0.0625;
