...
export function AircraftLayer({
  ...
  // Mount datasource and click handler
  useEffect(() => {
    if (!viewer) return;
  ...
  useEffect(() => {
    ...
    const processChunk = (): void => {
      ...
      for (let i = cursor; i < end; i++) {
        ...
        const position = Cartesian3.fromDegrees(ac.lon, ac.lat, ac.altitude_m);
      ...
      if (cursor < visible.length) {
        ...
        console.log(
          `[AircraftLayer] render complete — ${visible.length} entities on globe`,
        );
      }
    };
    ...
    if (visible.length > 0) {
      console.log(
        `[AircraftLayer] starting chunked render: ${visible.length} aircraft (chunk size: ${CHUNK_SIZE})`,
      );
      rafId = requestAnimationFrame(processChunk);
    }
